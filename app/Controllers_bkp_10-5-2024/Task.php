...
class Task extends BaseController
{
  ...
  public function list()
  {
    ...
    /*
        ...
        */
    $Output = $this->db->table('task')
      ->select('task.*,employees.first_name,employees.last_name')
      ->join('employees', 'employees.id = task.employee_id')
            ...
            foreach($start_end_detail as $start_end_detail_val){
              ...
              if($start_time != "0000-00-00 00:00:00"){
                if($end_time == "0000-00-00 00:00:00"){
                  ...
                  $start_end_html = '<a href="'.url_to('Task::addStartEnd').'?p='.$Output_value['id'].'&main='.$start_end_detail_val['id'].'&action_flage=end" class="btn btn-sm btn-danger bg-gradient-primary btn-icon-split mt-4 float-right rounded-0">End Time </a>';

                }else{
                  $timing .= $start_time.'===='.$end_time.'<br>';
                  $start_end_html = '<a href="'.url_to('Task::addStartEnd').'?p='.$Output_value['id'].'&main='.$start_end_detail_val['id'].'&action_flage=start" class="btn btn-sm btn-success bg-gradient-primary btn-icon-split mt-4 float-right rounded-0">Start Time </a>';
                }
              }
            }
          }else{
            $start_end_html = '<a href="'.url_to('Task::addStartEnd').'?p='.$Output_value['id'].'&main=0&action_flage=start" class="btn btn-sm btn-success bg-gradient-primary btn-icon-split mt-4 float-right rounded-0">Start Time </a>';
          }
  ...
  public function searchDepartment()
  {
    ...
    public function addTask()
    {
        if ($this->request->is('post')) {
            ...
            return redirect()->to(url_to('Task::list'));
        } else {
            $title = "Add New Task";
