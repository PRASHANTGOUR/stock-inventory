...
foreach ($products as $product) {
    if($product['all_employee'] == 1){
        ...
        $unique_value = strtotime($product['start_date']).'_'.strtotime($product['end_date']);
        $sched_data[$unique_value]=$product;
    }else{
        $product['first_name'] = srting_decrypt($product['first_name']);
        $product['last_name'] = srting_decrypt($product['last_name']);
        $sched_data[]=$product;
    }
}
		...
		var calendar = new Calendar(calendarEl, {
                        ...
                        events:function(event,successCallback){
							...
							Object.keys(scheds).map(k=>{
                                selected_end_date = moment(scheds[k].end_date).format("YYYY-MM-DD");// HH:mm
                                var new_date = moment(selected_end_date, "YYYY-MM-DD").add('d', 1);
                                var day = new_date.format('DD');
                                var month = new_date.format('MM');
                                var year = new_date.format('YYYY');
                                selected_end_date = year+'-'+month+'-'+day;
                                events.push({
...
function fun_change_department(department_id){
    ...
    window.location.href = '<?php echo url_to('Calendaremployees::list');?>?department_id='+department_id;
